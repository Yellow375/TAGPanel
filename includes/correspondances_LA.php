<?php
if(isset($arretsuivant1)){
    if($stops[$arretsuivant1] == "L'Etoile"){ ?>
        <div class="corresp0">
            <img src="correspondances/Ligne_C2.png" alt="Ligne C2" class="imagecorresp"> 
            <img src="correspondances/Ligne_25.png" alt="Ligne 25" class="imagecorresp">
            <img src="correspondances/Ligne_64.png" alt="Ligne 64" class="imagecorresp">
            <img src="correspondances/Ligne_CR.png" alt="Cars Région" class="imagecorresp">
            <img src="correspondances/parkingrelais.png" alt="Parking Relais" class="imagecorresp">
        </div>
        <?php }elseif($stops[$arretsuivant1] == "Denis Papin"){ ?>
        <div class="corresp0">
            <img src="correspondances/Ligne_66.png" alt="Ligne 66" class="imagecorresp"> 
        </div>
        <?php }elseif($stops[$arretsuivant1] == "Auguste Delaune"){ ?>
    <div class="corresp0">
        <img src="correspondances/Ligne_C3.png" alt="Ligne C3" class="imagecorresp">
        <img src="correspondances/Ligne_16.png" alt="Ligne 16" class="imagecorresp">
        <img src="correspondances/Ligne_66.png" alt="Ligne 66" class="imagecorresp">
    </div>
    <?php }elseif($stops[$arretsuivant1] == "Marie Curie"){ ?>
        <div class="corresp0">
            <img src="correspondances/Ligne_16.png" alt="Ligne 16" class="imagecorresp">
            <img src="correspondances/Ligne_66.png" alt="Ligne 66" class="imagecorresp">
        </div>
        <?php } elseif($stops[$arretsuivant1] == "La Rampe - Centre Ville"){ ?>
    <div class="corresp0">
        <img src="correspondances/Ligne_C7.png" alt="Ligne C7" class="imagecorresp"> 
        <img src="correspondances/Ligne_66.png" alt="Ligne 66" class="imagecorresp"> 
        <img src="correspondances/Ligne_68.png" alt="Ligne 68" class="imagecorresp">
    </div>
    <?php } elseif($stops[$arretsuivant1] == "Echirolles Gare"){ ?>
        <div class="corresp0">
        <img src="correspondances/SNCF.png" alt="TER" class="imagecorresp"> 
        <img src="correspondances/parkingrelais.png" alt="Parking Relais" class="imagecorresp">
    </div>
    <?php }elseif($stops[$arretsuivant1] == "Essarts - La Butte"){ ?>
        <div class="corresp0">
        <img src="correspondances/Ligne_C6.png" alt="Ligne C6" class="imagecorresp">
    </div>
    <?php }elseif($stops[$arretsuivant1] == "Polesud - Alpexpo"){ ?>
        <div class="corresp0">
            <img src="correspondances/Ligne_C3.png" alt="Ligne C3" class="imagecorresp"> 
            <img src="correspondances/Ligne_65.png" alt="Ligne 65" class="imagecorresp">
            <img src="correspondances/Ligne_67.png" alt="Ligne 67" class="imagecorresp">
        </div>
    <?php }elseif($stops[$arretsuivant1] == "Grand'Place"){ ?>
        <div class="corresp0">
            <img src="correspondances/Ligne_C3.png" alt="Ligne C3" class="imagecorresp">
            <img src="correspondances/Ligne_C6.png" alt="Ligne C6" class="imagecorresp">
            <img src="correspondances/Ligne_12.png" alt="Ligne 12" class="imagecorresp">
            <img src="correspondances/Ligne_65.png" alt="Ligne 65" class="imagecorresp">
            <img src="correspondances/Ligne_67.png" alt="Ligne 67" class="imagecorresp">
            <img src="correspondances/AM.png" alt="Agence de Mobilité" class="imagecorresp">
        </div>
    <?php }elseif($stops[$arretsuivant1] == "Malherbe"){ ?>
        <div class="corresp0">
        <img src="correspondances/Ligne_C5.png" alt="Ligne C5" class="imagecorresp">
     </div>
    <?php }elseif($stops[$arretsuivant1] == "Chavant"){ ?>
        <div class="corresp0">
            <img src="correspondances/Ligne_C.png" alt="Ligne C" class="imagecorresp"> 
            <img src="correspondances/Ligne_C1.png" alt="Ligne C1" class="imagecorresp">
            <img src="correspondances/Ligne_C4.png" alt="Ligne C4" class="imagecorresp">
            <img src="correspondances/Ligne_13.png" alt="Ligne 13" class="imagecorresp">
            <img src="correspondances/Ligne_CR.png" alt="Cars Région" class="imagecorresp">
        </div>
    <?php }elseif($stops[$arretsuivant1] == "Verdun - Préfecture"){ ?>
        <div class="corresp0">
            <img src="correspondances/Ligne_12.png" alt="Ligne 12" class="imagecorresp">
            <img src="correspondances/Ligne_13.png" alt="Ligne 13" class="imagecorresp">
            <img src="correspondances/Ligne_14.png" alt="Ligne 14" class="imagecorresp">
            <img src="correspondances/Ligne_15.png" alt="Ligne 15" class="imagecorresp">
            <img src="correspondances/Ligne_16.png" alt="Ligne 16" class="imagecorresp">
            <img src="correspondances/Ligne_CR.png" alt="Cars Région" class="imagecorresp">
        </div>
    <?php }elseif($stops[$arretsuivant1] == "Hubert Dubedout - Maison du Tourisme"){ ?>
        <div class="corresp0">
            <img src="correspondances/Ligne_B.png" alt="Ligne B" class="imagecorresp">
        </div>
    <?php }elseif($stops[$arretsuivant1] == "Victor Hugo"){ ?>
        <div class="corresp0">
            <img src="correspondances/Ligne_B.png" alt="Ligne B" class="imagecorresp">
            <img src="correspondances/Ligne_C1.png" alt="Ligne C1" class="imagecorresp">
            <img src="correspondances/Ligne_C3.png" alt="Ligne C3" class="imagecorresp">
            <img src="correspondances/Ligne_C4.png" alt="Ligne C4" class="imagecorresp">
            <img src="correspondances/Ligne_40.png" alt="Ligne 40" class="imagecorresp">
            <img src="correspondances/Ligne_CR.png" alt="Cars Région" class="imagecorresp">
        </div>
    <?php }elseif($stops[$arretsuivant1] == "Alsace-Lorraine"){ ?>
        <div class="corresp0">
        <img src="correspondances/Ligne_B.png" alt="Ligne B" class="imagecorresp"> 
            <img src="correspondances/Ligne_E.png" alt="Ligne E" class="imagecorresp">
            <img src="correspondances/Ligne_26.png" alt="Ligne 26" class="imagecorresp">
            <img src="correspondances/Ligne_CR.png" alt="Cars Région" class="imagecorresp">
            <img src="correspondances/AM.png" alt="Agence de Mobilité" class="imagecorresp">
        </div>
    <?php }elseif($stops[$arretsuivant1] == "Gares"){ ?>
        <div class="corresp0">
        <img src="correspondances/Ligne_B.png" alt="Ligne B" class="imagecorresp"> 
        <img src="correspondances/Ligne_C1.png" alt="Ligne C1" class="imagecorresp"> 
        <img src="correspondances/Ligne_26.png" alt="Ligne 26" class="imagecorresp">
        <img src="correspondances/Ligne_CR.png" alt="Cars Régions" class="imagecorresp">
        <img src="correspondances/AM.png" alt="Agence de Mobilité" class="imagecorresp"> 
        <img src="correspondances/SNCF.png" alt="TER" class="imagecorresp">
        <img src="correspondances/veloplus.png" alt="M Vélo+" class="imagecorresp">
        </div>
    <?php } elseif($stops[$arretsuivant1] == "Saint-Bruno"){ ?>
        <div class="corresp0">
        <img src="correspondances/Ligne_B.png" alt="Ligne B" class="imagecorresp">
        </div>
    <?php }elseif($stops[$arretsuivant1] == "Berriat - Le Magasin"){ ?>
        <div class="corresp0">
        <img src="correspondances/Ligne_C5.png" alt="Ligne C5" class="imagecorresp"> 
        </div>
    <?php }elseif($stops[$arretsuivant1] == "Louis Maisonnat"){ ?>
        <div class="corresp0">
        <img src="correspondances/Ligne_C6.png" alt="Ligne C6" class="imagecorresp"> 
            <img src="correspondances/Ligne_19.png" alt="Ligne 19" class="imagecorresp">
            <img src="correspondances/Ligne_CR.png" alt="Cars Région" class="imagecorresp">
        </div>
    <?php }elseif($stops[$arretsuivant1] == "La Poya"){ ?>
        <div class="corresp0">
        <img src="correspondances/Ligne_19.png" alt="Ligne 19" class="imagecorresp"> 
            <img src="correspondances/Ligne_20.png" alt="Ligne 20" class="imagecorresp">
            <img src="correspondances/Ligne_50.png" alt="Ligne 50" class="imagecorresp">
            <img src="correspondances/Ligne_CR.png" alt="Cars Région" class="imagecorresp">
        </div>
    <?php }else { ?>
<div class="nocorresp"></div>
<?php }} ?>